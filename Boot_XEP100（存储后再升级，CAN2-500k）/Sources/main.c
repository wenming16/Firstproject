...
void main(void) 
{
  ...
  (*boot) = 0x66;
  //(*DataIVBR) = 0x77;
  if((*DataIVBR) == 0x77)//需要再应用程序中增加一个地址变量进入BOOT或是升级中断后进入原先的AAP中
  {
    (*DataIVBR) = 0x11;
  }
  ...
  { 
    switch(caseflag)
    {
      ...
      case 1:
        ...
        EraseFlash_Status(0x80, EraseFlash_Upgrade);//擦除代码段flash(此处之后不因掉电)
        CAN_SendByte(0xC4);  //代码下载完毕
        QQ[3]++;
        PITINTE_PINTE0 = 1;   //开启数据读取中断(PIT0)
...
{  
   ...
   for(i=0; i<16; i++)//此处最大为16个个循环,改大会导致队列存储溢出
   {
     ...
     {
        RxBAvail--;
        RxBuff[RxIn++] = (UINT8)(data>>8);      //place the received byte in the buffer
        RxBAvail--;
