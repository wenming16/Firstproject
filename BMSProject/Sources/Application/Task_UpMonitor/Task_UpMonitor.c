...
{  
  ...
  for(i=0;i<2;i++)
  {
    ...
    switch(i)
    {
      case 0:    //电池总压及压差
      	BMS_to_Upmonitor->m_data[0] = i;     //编号 0x00
      	BMS_to_Upmonitor->m_data[1] = (uint8)(g_DataColletInfo.SysVolt_Total*0.001);     //电池总压（单体累加）   0.1V分辨率
  ...
  BMS_to_Upmonitor->m_data[4] = (uint8)(g_InsulInfo.Insul_Resis_Pos*10);           //正对地电阻值   0.1V分辨率
  BMS_to_Upmonitor->m_data[5] = ((uint16)(g_InsulInfo.Insul_Resis_Pos*10))>>8;               
  BMS_to_Upmonitor->m_data[6] = (uint8)(g_InsulInfo.Insul_Resis_Neg*10);            //负对地电阻值  0.1V分辨率（未写）
    ...
    switch(i)
    {
      case 0:     
      	BMS_to_Upmonitor->m_data[0] = i;     //编号  0x00
      	  ...
      	  BMS_to_Upmonitor->m_data[1] = (Positive & 0x03) + ((0x01 << 2) & 0x0C) + ((0x01 << 4) & 0x30) + ((0x01 << 6) & 0xC0);      //继电器状态 0 打开，1关闭
  ...
  */
  ...
  BMS_to_Upmonitor.m_ID = 0x1898C0F4;
	...
	BMS_to_Upmonitor.m_data[4] = (uint8)(g_SOCInfo.SOC_ValueRead*100);                 //输出SOC值（分辨率：1%）
	BMS_to_Upmonitor.m_data[5] = (uint8)(g_SOCInfo.SOC_ValueVoltGet*100);                 //按电压计算的SOC值（分辨率：1%）
	BMS_to_Upmonitor.m_data[6] = (uint8)(g_SOCInfo.SOC_Init);                          //程序错误1:错误;0:无错
	BMS_to_Upmonitor.m_data[7] = ((uint16)(g_SOCInfo.SOC_Init))>>8;                                           //备用
  Return_Value = BMS_CAN3SendMsg_Monitor(&BMS_to_Upmonitor);  
  */
