...
========================================================================*/
 ...
 PowerOff_StartTime_T PowerOff_StartTime;//休眠前开始的计时变量
 /*=======================================================================
...
void PowerOnOff_Control(uint8 FltLevel, uint8 FltSeconds, uint8 StaticCurrSet, uint8 StaticTimeSet)
{
   ...
   else//非2级故障,只要满足电流小于2A就任务其即将进入休眠状态
   {
      ...
      if(Sleep_StaticTime(Read_IIC_Time.IIC_Read_Hour, Read_IIC_Time.IIC_Read_Minute,\
         DataColletInfo.DataCollet_Current_Filter, StaticCurrSet,\
         &PowerOff_StartTime.SwitchOff_StartTime,  StaticTimeSet))
      {
         PositiveRelay_Control(Relay_OFF);
...
{
   PowerOnOff_Control(Fault_DisCharge.Level_DisCharge_SwitchOff_flag|Fault_Charge.Level_Charge_SwitchOff_flag, 30, 2, 12);
}