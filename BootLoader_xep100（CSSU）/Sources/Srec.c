...
static UINT8 GetHexByte(UINT8 *MemByte)    //将十进制转化为两位16进制的数
{
  ...
  *MemByte = 0;
  ...
  {
    return(0xA1);  //no. return an error
  }
  ...
  *MemByte = (isdigit(c) ? c - '0' : c - 'A' + 10) << 4;
  ...
  {
    return(0xA2);  //no. return an error
  }
  //convert the ASCII character to a binary hex nybble
...
{
  if(ptr->Lenth<DtaLenth)
  {
    //ww[0]=ptr->Lenth;
    ptr->DATA[ptr->Lenth++] = data;
    return 0;
  }
  else 
  {
...
UINT8 Data_OUT(RecData_T*ptr)
{
  ...
  if(ptr->Lenth>0)
  {
    data = ptr->DATA[--ptr->Lenth];
    return data;
  } 
}

/******************************************************************************/
static UINT8 GetHexByte_Store(UINT8 *MemByte,RecData_T*dataptr)    //将十进制转化为两位16进制的数
{
  INT8 c;           //used to hold the received ASCII hex byte   保存16进制字节的ASCII码
...
/******************************************************************************/
UINT8 RcvSRecord_Store(SRecDataRec *SRecData,RecData_T*dataptr)      //接收S记录文档
{ 
  UINT8 Error;
  ...
  for(;;)
  {
    ...
    {
      if(Data_IN(dataptr,'S')==1)
      {
        return 0x21;
      }
      if(Data_IN(dataptr,RecType)==1)//存储S19文件中前2个字节
      {
        return 0x21;
      }
      ...
      Error = GetHexByte_Store(&SRecData->NumBytes, dataptr);
      if (Error != noErr)       //get the record length field. valid hex byte?接收到一个有效16进制的数   
      {
        return(0x22);          //no. return an error
      }
      ...
      {
        return(0x23);         //yes. report an error
      }
      
      for (x = 0; x < AddrBytes; x++)         //字节地址
      {
        Error = GetHexByte_Store(&SRecByte, dataptr);
        
        if (Error != noErr)     //get a byte of the load address. valid hex byte?
        {
          return(0x24);        //no return an error
        }
      ...
      for (x = 0; x < NumDataBytes; x++)  //receive the data field接收到S记录中的数据字节数
      {
        ...
        {
          return(0x25);        //no return an error
        }
...
/******************************************************************************/
UINT8 RcvSRecord_Upgrade(SRecDataRec *SRecData)      //接收S记录文档
{ 
  UINT8 Error;
    //used to hold the return error code
  UINT8 CheckSum;
  ...
  for(;;)
  {
    ...
    else 
    {
      ...
      if (Error != noErr)       //get the record length field. valid hex byte?接收到一个有效16进制的数   
        return(0x91);          //no. return an error
      ...
      if (NumDataBytes > MaxSRecLen)  //is the S-Record longer than allowed?
      {
        return(0x92);         //yes. report an error
      }
      ...
      for (x = 0; x < AddrBytes; x++)         //字节地址
      {
        ...
        if (Error != noErr)     //get a byte of the load address. valid hex byte?
          return(0x93);        //no return an error
        CheckSum += SRecByte;   //add it into the checksum
        LoadAddress = (LoadAddress << 8) + SRecByte;
      } 
      ...
      for (x = 0; x < NumDataBytes; x++)  //receive the data field接收到S记录中的数据字节数
      {
        Error = GetHexByte(&SRecByte);
        if (Error != noErr)     //get a data field byte. valid hex byte?
          return(0x94);        //no return an error
        CheckSum += SRecByte;   //yes. add it in to the checksum
      ...
      //get the record checksum (last byte of the record). valid hex byte?最后一个校验位字节
      if (Error != noErr)
        return(0x95);          //no. return an error
        
      CheckSum = ~CheckSum;     //take 1's compliment of calculated checksum
      if (CheckSum != SRecByte) //calculated checksum == received checksum ?
      {
        return(0x96);   //no. return an error
      }
      else 
      {
        return(noErr);
