...
void INIT_CAN2(void) 
{
  ...
  CAN2IDAR1=0x20;    //若为子板(0x002)则此值为0x40
  CAN2IDAR2=0x00; 
  CAN2IDAR3=0x00;
...
Bool MSCAN2GetMsg(struct can_msg *msg)
{
  ...
  // 读标识符
  msg->id = (unsigned int)(CAN2RXIDR0<<3)|(unsigned char)(CAN2RXIDR1>>5); 
...
Bool MSCAN2GetMsg(struct can_msg *msg)
{
  ...
  if (CAN2RXIDR1_IDE == 0)                     // 收到标准帧；
	{
		msg->id = (unsigned long)(CAN2RXIDR0<<3) | (unsigned long)(CAN2RXIDR1>>5);
    msg->m_IDE = 0;
	} 
	else                                         // 收到扩展帧；
	{
	 	msg->id = (((unsigned long)CAN2RXIDR0)<<21)|((unsigned long)(CAN2RXIDR1&0xe0)<<13)|((unsigned long)(CAN2RXIDR1&0x07)<<15)|(((unsigned long)CAN2RXIDR2)<<7);
    msg->id = msg->id | ((unsigned long)(CAN2RXIDR3&0xfe)>>1);
    msg->m_IDE = 1;
	}
...
void CAN_SendByte(UINT8 data)
{
  can_send.data[0] = data;
...
{  
   ...
   {
     if(can_msg.id == ID_CSSU1) //根据ID号进行解码CAN通信数据
     {
     ...
     else if(can_msg.id == Boot_ID)//在上位机发送0xF300后Boot重新启动发送Boot启动帧0xC0
     {                             //此时MCU中不管有没有应用程序都会重新启动
        if((can_msg.data[0] == (ID_CSSU1-0x01)) && (can_msg.data[1] == 0xAA))
        {
